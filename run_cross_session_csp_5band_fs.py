# run_cross_session_csp_5band_fs.py

import csv
import os
import re
import sys

import numpy as np

import config
from train_csp_5band_fs_baseline import (
    print_final_summary,
    run_loso_csp_5band_fs_baseline,
    sort_session_ids,
)


# =========================================================
# 1) KLASOR YOLLARI
# =========================================================

WINDOW_FOLDER = os.path.join(config.OUTPUT_DIR, "window_data_wideband")
RESULTS_FOLDER = os.path.join(config.OUTPUT_DIR, "baseline_results")


# =========================================================
# 2) BASIT CLI
# =========================================================

def get_session_range_from_cli():
    """
    Opsiyonel session araligini alir.

    Kullanim:
    python run_cross_session_csp_5band_fs.py
    python run_cross_session_csp_5band_fs.py 1 3
    """
    if len(sys.argv) == 1:
        return None, None

    if len(sys.argv) == 3:
        start_session = int(sys.argv[1])
        end_session = int(sys.argv[2])

        # Aralik secimi kucuk deneme kosulari icindir; session sirasi korunur ve karistirilmaz.
        if start_session > end_session:
            raise ValueError("Baslangic session, bitis session'dan buyuk olamaz.")

        return start_session, end_session

    raise ValueError("Kullanim: python run_cross_session_csp_5band_fs.py [start_session end_session]")


# =========================================================
# 3) WINDOW DOSYALARINI BUL
# =========================================================

def get_session_window_files():
    """
    outputs/window_data_wideband klasoru icindeki session window dosyalarini bulur.
    """
    if not os.path.exists(WINDOW_FOLDER):
        raise FileNotFoundError(f"Window klasoru bulunamadi: {WINDOW_FOLDER}")

    session_files = {}

    for filename in os.listdir(WINDOW_FOLDER):
        if filename.endswith("_wideband_windows.npz") and filename.startswith("session_"):
            match = re.search(r"session_(\d+)_wideband_windows\.npz", filename)

            if match:
                session_id = match.group(1)
                full_path = os.path.join(WINDOW_FOLDER, filename)
                session_files[session_id] = full_path

    if len(session_files) == 0:
        raise ValueError("Hic wideband window dosyasi bulunamadi.")

    return session_files


# =========================================================
# 4) SESSION WINDOW YUKLE
# =========================================================

def load_one_session_window(file_path):
    """
    Tek bir session'in wideband window dosyasini okur.
    """
    data = np.load(file_path)
    X = data["X"]
    y = data["y"]

    return X, y


def load_all_sessions(start_session=None, end_session=None):
    """
    Tum mevcut session wideband window dosyalarini yukler.
    """
    session_files = get_session_window_files()
    sorted_ids = sort_session_ids(list(session_files.keys()))

    # Feature selection train fold'u icinde fit edilecegi icin burada sadece ham wideband X/y yuklenir.
    # Test session bu yukleme adimindan sonra LOSO fold mantigiyla ayrilir.
    if start_session is not None and end_session is not None:
        sorted_ids = [
            session_id for session_id in sorted_ids
            if start_session <= int(session_id) <= end_session
        ]

        if len(sorted_ids) == 0:
            raise ValueError("Verilen aralikta wideband window dosyasi bulunamadi.")

    session_data = {}

    for session_id in sorted_ids:
        X, y = load_one_session_window(session_files[session_id])

        session_data[session_id] = {
            "X": X,
            "y": y
        }

    return session_data


# =========================================================
# 5) SONUCLARI KAYDET
# =========================================================

def save_fold_results(fold_results):
    """
    Fold sonuclarini CSV olarak kaydeder.
    """
    # FS + LDA sonuclari, k degeri ve fold bilgisiyle birlikte raporlanabilir kalir.
    # Bu CSV ablation ozetleri icin dogrudan girdi olarak kullanilir.
    if len(fold_results) == 0:
        raise ValueError("Kaydedilecek fold sonucu yok.")

    os.makedirs(RESULTS_FOLDER, exist_ok=True)
    save_path = os.path.join(RESULTS_FOLDER, "loso_csp_5band_fs_lda_results.csv")
    fieldnames = list(fold_results[0].keys())

    with open(save_path, "w", newline="", encoding="utf-8") as f:
        writer = csv.DictWriter(f, fieldnames=fieldnames)
        writer.writeheader()
        writer.writerows(fold_results)

    print(f"\nFold sonuclari kaydedildi: {save_path}")


def save_predictions(all_predictions):
    """
    Test fold tahminlerini CSV olarak kaydeder.
    """
    # Pencere bazli skorlar, secilen feature sayisinin hangi test session'larda zorlandigini gormek icindir.
    if len(all_predictions) == 0:
        raise ValueError("Kaydedilecek tahmin yok.")

    os.makedirs(RESULTS_FOLDER, exist_ok=True)
    save_path = os.path.join(RESULTS_FOLDER, "loso_csp_5band_fs_lda_predictions.csv")
    fieldnames = list(all_predictions[0].keys())

    with open(save_path, "w", newline="", encoding="utf-8") as f:
        writer = csv.DictWriter(f, fieldnames=fieldnames)
        writer.writeheader()
        writer.writerows(all_predictions)

    print(f"Tahminler kaydedildi: {save_path}")


# =========================================================
# 6) ANA AKIS
# =========================================================

def main():
    start_session, end_session = get_session_range_from_cli()

    print("Session wideband window dosyalari araniyor...")
    session_data = load_all_sessions(start_session, end_session)

    session_ids = sort_session_ids(list(session_data.keys()))
    print("Bulunan session'lar:", session_ids)

    fold_results, all_predictions = run_loso_csp_5band_fs_baseline(session_data, verbose=True)

    print_final_summary(fold_results)
    save_fold_results(fold_results)
    save_predictions(all_predictions)

    print("\nCross-session 5-band CSP + FS kosusu tamamlandi.")


if __name__ == "__main__":
    main()
